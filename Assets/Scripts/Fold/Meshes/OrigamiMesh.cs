...
namespace Origami_Mesh
{
	...
	public sealed class OrigamiMesh : OrigamiBase, IFoldMeshCallbacks
	{
		...
		public int FoldLayer { get => m_vertices.VertexInfoList[1].Layer; }
		...
		public OrigamiMesh(in MeshVertexInfo info1, in MeshVertexInfo info2, in MeshVertexInfo info3, bool facing) : base(info1, info2, info3, facing)
		{
		...
		public void FoldOrigamiMeshByRadians(in OrigamiFoldResults foldRes, in OrigamiAdjustedVertexResults adjustRes, in float rad, in Matrix4x4 matZ)
		{
			...
			{
				if (MeshVertices.VertexInfoList[0].IsConnectedToCrease)
				{
					if (adjustRes.NeedsAdjustment)
					{
				...
				if (MeshVertices.VertexInfoList[1].IsConnectedToCrease)
				{
					if (adjustRes.NeedsAdjustment)
					{
				...
				if (MeshVertices.VertexInfoList[2].IsConnectedToCrease)
				{
					if (adjustRes.NeedsAdjustment)
					{
		...
		public void AdjustOrigamiMeshByRadians(in OrigamiAdjustedVertexResults adjustedVertexResults, in float rad)
		{
			...
			if (this.MeshVertices.VertexInfoList[0].IsConnectedToCrease) vec0 = FastLerp(this.MeshVertices[0], adjustedVertexResults.Result0, t);
			else vec0 = this.MeshVertices[0];

			if (this.MeshVertices.VertexInfoList[1].IsConnectedToCrease) vec1 = FastLerp(this.MeshVertices[1], adjustedVertexResults.Result1, t);
			else vec1 = this.MeshVertices[1];

			if (this.MeshVertices.VertexInfoList[2].IsConnectedToCrease) vec2 = FastLerp(this.MeshVertices[2], adjustedVertexResults.Result2, t);
