...
namespace Origami_Result
{
	...
	public readonly struct OrigamiFoldResults : IFoldResults
	{
		...
		)
		{
			...
			//交点を求める
			{
				...
				Point0_Result = new OrigamiFoldResult(mesh, vertices[0], midPoint, CreaseOffset, matX, matZ, vertices.VertexInfoList[0].IsConnectedToCrease);

				midPoint = OrigamiUtility.GetPerpendicularIntersectionPoint(vertices[1], creasePoint, creaseVec, magnitude);
				Point1_Result = new OrigamiFoldResult(mesh, vertices[1], midPoint, CreaseOffset, matX, matZ, vertices.VertexInfoList[1].IsConnectedToCrease);

				midPoint = OrigamiUtility.GetPerpendicularIntersectionPoint(vertices[2], creasePoint, creaseVec, magnitude);
				Point2_Result = new OrigamiFoldResult(mesh, vertices[2], midPoint, CreaseOffset, matX, matZ, vertices.VertexInfoList[2].IsConnectedToCrease);
	...
	public readonly struct OrigamiAdjustedVertexResults
	{
		...
		public OrigamiAdjustedVertexResults(in OrigamiMesh mesh, in float rad, in Vector3 startPoint, in Vector3 closestIntersection, in Vector3 dir, in Vector3 baseDir, in float baseMagnitude, in float baseSqrMagnitude, in Matrix4x4 matZ)
		{
			...
			{
				Vector3 v;
				if (vertices.VertexInfoList[i].IsConnectedToCrease)
				{
		...
		{
			if (mesh.MeshVertices.VertexInfoList[0].IsConnectedToCrease)
			{
				Result0.z = res.Point0_Result.FoldResult.FoldOriginalPoint90.z;
			}
			if (mesh.MeshVertices.VertexInfoList[1].IsConnectedToCrease)
			{
				Result1.z = res.Point1_Result.FoldResult.FoldOriginalPoint90.z;
			}
			if (mesh.MeshVertices.VertexInfoList[2].IsConnectedToCrease)
			{
